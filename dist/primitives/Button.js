 ...
 */
export const Button = forwardRef(function Button({ variant = 'ghost', size = 'md', icon = false, active, className, type = 'button', ...rest }, ref) {
    return (_jsx("button", { ref: ref, type: type, "aria-pressed": active, className: cx('pill', variant, size === 'sm' && 'small', icon && 'gear', active && 'on', className), ...rest }));
});
