...
function stamp(fit, source, filter, offset = 0) {
    const { ctx } = fit;
    ctx.save();
...
function drawLayer(canvas, fit, env, layer) {
    const { ctx, dpr } = fit;
 ...
 */
function drawLayers(base, bright, env) {
...
function readsSamples({ peaks, buffer, viewStart, viewEnd }) {
...
function drawColumns(ctx, w, h, env) {
...
export function Waveform({ peaks, buffer, scale, color, viewStart, viewEnd, bucketColors }) {
    ...
    useLayoutEffect(() => {
        const env = { peaks, buffer, scale, color, viewStart, viewEnd, bucketColors };
        const redraw = () => {
